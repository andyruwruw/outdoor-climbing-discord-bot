      ...
      for (let i = 0; i < CommandManager._commands.length; i += 1) {
        ...
        if (command.getKey() in applicationCommands) {
          if (CommandManager._applicationCommandMatches(
            ...
            await (client.application as ClientApplication).commands.delete(applicationCommands[command.getKey()].id);
          }
        }
  
        if (shouldCreateApplication) {
          await (client.application as ClientApplication).commands.create(command.create());
        }
      }
